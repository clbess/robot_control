...
class Blob:
    def __init__(self, mode_settings, precision, x_tar, y_tar, w_tar, h_tar, hsv_min, hsv_max):
        ...
        self.detection_pub = rospy.Publisher('/detection', Bool, queue_size=10)

        self.bool.data = False
        self.detection_pub.publish(bool)
            ...
            if area > 100:
                ...
                if ((self.x >= self.x_tar - self.precision) and (self.x <= self.x_tar + self.precision) and (self.w >= self.w_tar - 4 * self.precision) and (self.w <= self.w_tar + 4 * self.precision) and (self.h >= self.h_tar - self.precision) and (self.h <= self.h_tar + self.precision) and ((self.mode_settings == 0) or (self.mode_settings == 3))):
                    if (self.found == 0):
                        ...
                        self.bool.data = True
                        self.detection_pub.publish(self.bool)
        return img_Contour
    ...
    def ControlRobot(self):
        if(self.found == 1 or self.mode_settings == 1 or self.mode_settings == 2):
            ...
            self.linear_vel = 0.06
        else:
            if(self.x < self.x_tar - 3 * self.precision or self.x > self.x_tar + 3 * self.precision):
                self.angular_vel = math.copysign(0.03, self.x_tar - self.x)
            else:
                self.angular_vel = 0.0

            if(self.h < self.h_tar - 2 * self.precision or self.h > self.h_tar + 2 * self.precision):
                self.linear_vel = math.copysign(0.02, self.h_tar - self.h)
            else:
                self.linear_vel = 0.0

        '''elif(self.x < self.x_tar - 5 * self.precision or self.x > self.x_tar + 5 * self.precision):
               self.angular_vel = math.copysign(0.07, self.x_tar - self.x)
